# Pi-Guard Lite 使用说明

## 1. 系统用途

Pi-Guard Lite 用于：

- 接收 Suricata / 系统日志
- 解析事件
- 提取特征
- 结合规则、基线、ML 打分
- 自动生成策略
- 执行封禁
- 做探测与回滚

---

## 2. 常用脚本

### 2.1 生成演示模型

```powershell
python scripts\create_demo_models.py
```

### 2.2 处理 Suricata 样例

```powershell
python scripts\process_suricata_file.py tests\samples\suricata_eve.jsonl --apply-policy --run-probe
```

### 2.3 运行主链路服务

```powershell
python scripts\run_pipeline_service.py --source suricata --file tests\samples\suricata_eve.jsonl --mode existing
```

### 2.4 释放过期封禁

```powershell
python scripts\release_expired_blocks.py
```

### 2.5 导入模型

```powershell
python scripts\import_model.py anomaly models\anomaly_model.pkl --version demo-anomaly
python scripts\import_model.py classifier models\classifier_model.pkl --version demo-classifier
```

### 2.5.1 训练模型

```powershell
python scripts\train_models.py tests\samples\training_features.csv --out-dir models\trained --register --version sample-v1
```

### 2.6 Linux 运维脚本

```bash
bash scripts/bootstrap_rpi.sh
bash scripts/install_systemd.sh
bash scripts/install.sh
bash scripts/start.sh
bash scripts/stop.sh
bash scripts/status.sh
bash scripts/install_fluentbit_rpi.sh
bash scripts/verify_rpi_services.sh
bash scripts/backup.sh
```

### 2.6.1 联动设备检查

```bash
python scripts/check_executor.py
python scripts/check_executor.py --test-ip 203.0.113.200 --ttl 60
```

### 2.6.2 部署前检查

```bash
python scripts/preflight_check.py
python scripts/check_fluentbit_bridge.py
python scripts/collect_runtime_report.py
```

### 2.7 Fluent Bit 接入

```powershell
Get-Content tests\samples\fluentbit_syslog.jsonl | python scripts\run_fluentbit_stdin.py --no-policy --no-probe
```

真实环境可参考：

- `config/fluent-bit.conf`
- `config/fluent-bit-rpi.conf`

### 2.8 Fluent Bit 真机桥接

```bash
bash scripts/start_fluentbit_bridge.sh
```

### 2.8.1 自动扫描监听

```bash
python scripts/run_scan_listener_service.py --bind-host 0.0.0.0 --report-host 你的本机IP --ports 2201-2212
```

### 2.9 场景测试

```powershell
python scripts\run_scenario_tests.py
```

### 2.9.1 本机真实扫描演示

```powershell
python scripts\run_live_scan_demo.py
```

### 2.10 误伤检查

```powershell
python scripts\run_false_positive_check.py
```

### 2.11 稳定性检查

```powershell
python scripts\run_stability_check.py --iterations 10
```

---

## 3. 页面使用

### 首页
查看：
- 事件总数
- 策略总数
- 封禁总数
- 风险分布
- 监听器状态
- 最近事件
- 最近策略

### 事件页
查看：
- 事件类型
- 源/目的 IP
- 异常分
- ML 分
- 风险等级

点击事件 ID 可进入事件详情页。
支持：
- 搜索
- 自动刷新
- 分页
- 排序

### 高级分析页
查看：
- findings
- MITRE 映射
- 攻击图节点/边
- 受影响资产

页面入口：

- `/analysis/view/{event_id}`

### 策略页
查看：
- 策略动作
- 策略目标
- TTL
- 状态

点击策略 ID 可进入策略详情页。
支持：
- 搜索
- 自动刷新
- 分页
- 排序

### 黑名单页
查看：
- 被封禁 IP
- 原因
- 过期时间
- 状态

点击黑名单 ID 可进入黑名单详情页。
支持：
- 搜索
- 自动刷新
- 分页
- 排序

### 资产页
查看：
- IP
- 主机名
- 资产类型
- 重要性
- 负责人

点击资产 IP 可进入资产详情页。
支持：
- 搜索
- 分页
- 排序

### 探测页
查看：
- 探测总数
- 成功数
- 失败数
- 各探测目标详细记录
支持：
- 搜索
- 自动刷新
- 分页
- 排序

### 扫描监听页
查看：
- 自动监听状态
- 绑定地址
- 上报资产地址
- 监听端口
- 推荐扫描命令

页面入口：

- `/scan-listener/view`

### 运行状态页
查看：
- 主机信息
- 关键二进制
- systemd 状态
- 磁盘占用
- 温度

页面入口：

- `/runtime/view`

### 系统总览页
查看：
- 扫描监听状态
- 通知状态
- 模型状态
- 运行概况
- systemd 汇总

页面入口：

- `/system/view`

### 手工操作页
支持：
- 手工封禁
- 手工解封
- 查看最近策略
- 查看最近黑名单
- 查看执行器状态

### 模型管理页
支持：
- 查看模型版本
- 导入模型文件
- 激活指定版本

页面入口：

- `/models/view`

### 通知管理页
支持：
- 查看通知状态
- 发送测试通知

页面入口：

- `/notifications/view`

训练说明另见：

- `docs/高级攻击分析说明.md`
- `docs/模型训练与调优说明.md`
- `docs/通知管理说明.md`
- `docs/自动扫描监听说明.md`
- `docs/场景测试说明.md`
- `docs/本机扫描演示说明.md`
- `docs/稳定性测试说明.md`
- `docs/误伤与回滚测试说明.md`
- `docs/部署前检查说明.md`
- `docs/真机联调验收说明.md`
- `docs/树莓派运行检查说明.md`

---

## 4. JSON API

### 查询
- `GET /health`
- `GET /stats`
- `GET /events`
- `GET /api/events`
- `GET /analysis/event/{id}`
- `GET /policies`
- `GET /api/policies`
- `GET /blocklist`
- `GET /api/blocklist`
- `GET /assets`
- `GET /api/assets`
- `GET /api/probes`
- `GET /executor/check`
- `GET /probe/run`
- `GET /scan-listener/status`
- `GET /ml/status`
- `GET /models/evaluations`

### 操作
- `POST /manual/block`
- `POST /manual/unblock`
- `POST /ingest/raw`
- `POST /ingest/batch`

---

## 5. 手工操作示例

### 手工封禁

```json
POST /manual/block
{
  "ip": "203.0.113.50",
  "ttl_seconds": 1800,
  "reason": "manual block"
}
```

### 手工解封

```json
POST /manual/unblock
{
  "ip": "203.0.113.50",
  "reason": "manual unblock"
}
```

---

## 6. 运行建议

### Web 控制台交互
- 首页、事件、策略、黑名单、资产、探测页面已经支持：
  - 无刷新分页
  - 无刷新排序
  - 无刷新搜索
  - 定时自动刷新
- 控制台主题已改为统一静态资源：
  - `/static/device-console.css`
  - `/static/device-console.js`
- 首页、事件中心、策略中心和高级分析页已切换到新的模块化控制台布局
- 黑名单、资产、探测、系统总览、运行状态、扫描监听页也已切换到新的模块化布局
- 手工处置、模型管理、通知管理页也已统一到新的模块化控制台布局
- 详情页也已统一到新的设计系统，并开始使用共享模板宏组件
- 推荐使用 Chrome / Edge 访问控制台
- 登录默认账号：
  - 用户名：`admin`
  - 密码：`admin`
- 登录安全：
  - 连续失败达到阈值后会临时锁定
  - 会话 Cookie 具备过期时间
  - 登录成功/失败会写入审计日志
- 原始事件接入：
  - 支持通过 `/ingest/raw` 接收单条事件
  - 支持通过 `/ingest/batch` 接收批量事件
  - 当配置了 `ingest.auth_token` 时，需要在请求头携带 `x-pi-guard-token`
- 原始 JSONL 轮转：
  - `retention.raw_rotate_max_mb`
  - `retention.raw_rotate_max_lines`
- 模型效果验证：
  - 支持 `scripts/evaluate_models.py`
  - 支持在模型管理页查看最近评估记录

### 开发环境
- `dry_run=true`
- probe 使用本地健康检查
- 使用 demo 模型

### 真机环境
- `dry_run=false`
- 更换成真实 OpenWrt / 防火墙命令
- 配置真实 probe 目标
- 导入真实训练模型

---

## 7. 排查建议

### 无事件
- 检查输入文件路径
- 检查解析器 source 是否匹配
- 检查原始日志是否写入 `data/raw/`

### 无策略
- 检查风险分是否达到阈值
- 检查白名单是否命中
- 检查 policy generator 输出

### 无封禁
- 检查 `executor.yaml`
- 检查 `dry_run`
- 检查 `/executor/check`

### 总是回滚
- 检查 `probe.default_targets`
- 检查 `/health`
- 检查 probe 目标可达性
